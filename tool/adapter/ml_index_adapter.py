...
class MLIndexAdapter(Adapter):
    ...
    def __parse_index_section(self, index_section: IndexSection) -> StoreServiceConfig:
        ...
        if index_section.kind == MLIndexKind.FAISS:
            ...
            url = f"{api_base}/indexes/{index_section.index}/docs/search?api-version={api_version}"
            secret = api_key
            if index_section.field_mapping:
                text_field = index_section.field_mapping.content
                vector_field = index_section.field_mapping.embedding
        else:
