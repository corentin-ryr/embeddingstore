...
class PineconeClient(Agent):
    ...
    def search_by_embedding(self,
                            ...
                            top_k: int = 5) -> List[SearchResultEntity]:
        ...
        for matched_vector in matched_vectors:
            ...
            if self.__config.text_field in matched_vector[query_response_metadata_key]:
                result.text = matched_vector[query_response_metadata_key][self.__config.text_field]
            else:
                self.__logger.info("Pinecone vector text is recommended to be provided in vector metadata with key {0},"
                                   " currently it's empty.".format(self.__config.text_field))
    ...
    def __get_namespace(self):
        if self.__config.collection is None:
    ...
    def __pinecone_query(self,
        ...
        try:
            query_response = requests.post(url=self.query_url, headers=headers, json=query_body, verify=False)
