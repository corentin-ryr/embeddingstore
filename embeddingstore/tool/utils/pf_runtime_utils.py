...
class PromptflowRuntimeUtils:
    ...
    def get_pf_context_info() -> Dict[str, Any]:
        ...
        if pf_operation_context:
            context.update(pf_operation_context.promptflow_info)
    ...
    def get_pf_context_info_for_telemetry() -> Dict[str, Any]:
        ...
        candidates_for_telemetry = {
            k.replace('-', '_'): v
            for k, v in pf_context_info.items()
        }

        return {
            k: v
            for k, v in candidates_for_telemetry.items()
            if k in telemetry_dimensions
        }

    @staticmethod
    def get_app_insight_conn_str() -> str:
        from promptflow.utils.executor_logger import TelemetryLogHandler
